...
export const ExamsView: React.FC<ExamsViewProps> = ({ examsQuery, timeFormat = '24h' }) => {
  ...
  if (examsQuery.isPending && exams.length === 0) {
    return (
      <div className="space-y-4 animate-in fade-in duration-300">
        <div className="h-14 bg-bgCard border border-borderColor rounded-2xl animate-pulse" />
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-36 bg-bgCard border border-borderColor rounded-2xl p-4 flex gap-4 animate-pulse">
  ...
  if (examsQuery.isError && exams.length === 0) {
    return (
      <div className="p-4 bg-orange-500/10 text-orange-500 border border-orange-500/20 rounded-2xl flex items-center gap-3 text-xs font-mono">
  ...
  if (exams.length === 0) {
    return (
      <div className="bg-bgCard border-2 border-dashed border-orange-500/30 rounded-2xl p-8 text-center space-y-3">
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      {/* 1. ASYMMETRICAL INTENTIONAL HEADER BANNER */}
      <div className="bg-bgCard border-2 border-borderColor/90 rounded-2xl p-4 sm:p-5 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="space-y-1">
            ...
            <h3 className="font-sans font-black text-textMain text-lg sm:text-xl tracking-tight leading-none">
              Seat Allocation & Schedule
            </h3>
            <p className="text-xs text-textMuted font-mono">
              Real-time venue hall and seating registry for active assessments.
            </p>
          </div>

          <div className="flex items-center gap-2 font-mono">
            <div className="px-3 py-1.5 rounded-xl bg-orange-500/10 border border-orange-500/30 text-orange-500 text-xs font-bold flex items-center gap-1.5">
              <Layers className="h-3.5 w-3.5" />
              <span>{exams.length} Courses</span>
            </div>
            {examTypes.length > 0 && (
              <div className="px-3 py-1.5 rounded-xl bg-bgPrimary border border-borderColor text-textMuted text-xs font-semibold">
                {examTypes.length} {examTypes.length === 1 ? 'Phase' : 'Phases'}
              </div>
            )}
          </div>
        </div>
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-xl text-xs font-mono font-bold transition-all cursor-pointer whitespace-nowrap border ${
              selectedType === 'all'
                ...
                : 'bg-bgCard text-textMuted border-borderColor hover:text-textMain hover:border-orange-500/40'
            }`}
          >
      ...
      {/* 3. EXAM TICKET CARDS: ASYMMETRICAL & HIGH READABILITY */}
      <div className="space-y-3.5">
        {filteredExams.map((exam: any, idx: number) => {
          ...
          return (
            <div
              key={idx}
              className="bg-bgCard border-2 border-borderColor/80 rounded-2xl shadow-xs overflow-hidden transition-all hover:border-orange-500/60 group"
            >
              <div className="flex flex-col md:flex-row">
                {/* Left Date Block */}
                <div className="bg-bgPrimary/70 border-b md:border-b-0 md:border-r border-borderColor p-3.5 sm:p-4 flex md:flex-col items-center justify-between md:justify-center md:w-28 shrink-0 text-center gap-1 relative">
                  <div className="hidden md:block w-1.5 h-full absolute left-0 top-0 bg-orange-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                  <div>
                  ...
                  {exam.exam_session && (
                    <span className="text-[10px] font-mono font-extrabold px-2 py-0.5 rounded-md bg-orange-500/15 text-orange-500 border border-orange-500/30 uppercase">
                      {exam.exam_session}
                    </span>
                  )}
                </div>
                ...
                <div className="p-4 sm:p-5 flex-1 space-y-2.5 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="text-xs font-mono font-black px-2.5 py-0.5 rounded-lg bg-orange-500/10 text-orange-500 border border-orange-500/30 uppercase">
                      {exam.course_code}
                    </span>
                    {exam.slot && (
                ...
                {/* Right Seating & Venue Box (Industrial High-Function Punch) */}
                <div className="border-t md:border-t-0 md:border-l border-borderColor bg-bgPrimary/40 p-3.5 sm:p-4 md:w-56 shrink-0 flex md:flex-col items-center md:items-stretch justify-between gap-3">
                  {/* Venue Block */}
                  <div className="flex items-center gap-2">
                  ...
                  {/* Seat Number Block */}
                  <div className="flex items-center md:justify-between gap-2.5 bg-bgCard border border-orange-500/30 rounded-xl px-3 py-1.5 md:py-2 shadow-xs">
                    <div>
                      <span className="text-[9px] font-mono font-black text-orange-500 uppercase tracking-wider block leading-none">
                        Seat No
                      </span>
                      <span className={`text-base sm:text-lg font-mono font-black block leading-none mt-0.5 ${hasSeat ? 'text-orange-500' : 'text-textMuted text-xs'}`}>
                        {exam.seat_no || 'TBA'}
                      </span>
                    </div>
                    <Armchair className="h-4 w-4 text-orange-500/70 shrink-0" />
                  </div>
                </div>
              </div>
            </div>
